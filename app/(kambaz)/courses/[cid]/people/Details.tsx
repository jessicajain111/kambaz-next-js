...
export default function PeopleDetails({ uid, onClose }: { uid: string | null; onClose: () => void }) {
  ...
  const saveUser = async () => {
    const trimmed = name.trim() || `${user.firstName ?? ""} ${user.lastName ?? ""}`.trim();
    const parts = trimmed.split(/\s+/);
    const firstName = parts[0] ?? user.firstName ?? "";
    const lastName = parts.length > 1 ? parts.slice(1).join(" ") : user.lastName ?? "";
    const toSave = { ...user, firstName, lastName };
    await client.updateUser(toSave);
    setUser(toSave);
    setEditing(false);
    onClose();
  };
  ...
  }, [editing, user._id, user.firstName, user.lastName]);
